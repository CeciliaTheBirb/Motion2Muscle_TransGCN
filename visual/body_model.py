...
class BodyModel(nn.Module):
    ...
    def __init__(
        ...
        # -- Load SMPL params --
        if ".npz" in bm_fname or ".pkl" in bm_fname:
            smpl_dict = np.load(bm_fname, encoding="latin1", allow_pickle=True)
        else:
        ...
        self.comp_register(
            "init_v_template", torch.tensor(smpl_dict["v_template"][None], dtype=dtype), persistent=persistant_buffer
        )

        self.comp_register(
            "f", torch.tensor(smpl_dict["f"].astype(np.int32), dtype=torch.int32), persistent=persistant_buffer
        )

        num_total_betas = smpl_dict["shapedirs"].shape[-1]
        if num_betas < 1:
            num_betas = num_total_betas
        print("shapedirs shape:", smpl_dict["shapedirs"].shape)
        ...
        self.comp_register(
            "J_regressor", torch.tensor(smpl_dict["J_regressor"], dtype=dtype), persistent=persistant_buffer
        )
        ...
        self.comp_register(
            "kintree_table", torch.tensor(kintree_table, dtype=torch.int32), persistent=persistant_buffer
        )
        ...
        if self.model_type in ["smpl"]:
            ...
            self.comp_register(
                "init_pose_hand", torch.zeros((1, 15 * 3 * 2), dtype=dtype), persistent=persistant_buffer
            )
